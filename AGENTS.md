...
- **Known, accepted cost of the runtime time scale**: carrying the scale as an enum discriminant makes `Time` 24 bytes rather than 16 (and `TimeInterval` 40 rather than 32), which costs ~13% on the four `visibility_single_pair*` benchmarks — cache and memory traffic, not instruction count or dispatch. Accepted deliberately; the events2 adaptive sampler recoups it by cutting the number of detector evaluations. Note local `cargo bench` wall-time cannot see this, only CodSpeed's simulated-cache instrument can.
