...
- **Where the time scale is tracked**: the orbit layer does *not* parametrise on it — an orbit's epoch is a runtime-scaled `Time`. The scale stays in the type system only where it buys something: `lox-frames` rotations (which are valid only in a specific scale), `lox-time`'s offset graph, and generic utilities like `lox-analysis::events`. `lox-analysis` keeps an internal TAI axis and converts at ingest, which is O(1) since a `Trajectory` holds one epoch plus relative offsets.
