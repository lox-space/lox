...
- **Where the time scale is tracked**: neither the orbit nor the analysis layer parametrises on it — epochs there are runtime-scaled `Time`, and `lox-analysis` does not reference `Tai` at all. The scale stays in the type system only in the physics core: `lox-frames` rotations (valid only in a specific scale) and `lox-time`'s offset graph. `Sgp4` keeps a native `Time<Tai>` inherent API and converts at its `Propagator` boundary. Note `Ord for Time` *asserts* matching scales, so comparing times from different scales panics rather than failing to compile; interval algebra over mixed-scale times is a runtime error.
