...
- **Where the time scale is tracked**: neither the orbit nor the analysis layer parametrises on it — epochs there are runtime-scaled `Time`. The scale stays in the type system only in the physics core: `lox-frames` rotations (valid only in a specific scale) and `lox-time`'s offset graph. Note `Ord for Time` *asserts* matching scales, so comparing times from different scales panics rather than failing to compile; interval algebra over mixed-scale times is a runtime error.
- **`Scenario` is the one TAI island**: `Scenario` holds a `TimeInterval<Tai>` (its declared analysis window) and converts at the detection boundary. Everything downstream of it is runtime-scaled.
