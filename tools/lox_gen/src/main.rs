...
pub fn main() {
    ...
    let tasks: Vec<Task> = vec![
        ("sun", &SUN, vec![point_mass, spheroid, rotational_elements]),
        ("barycenters", &BARYCENTERS, vec![point_mass]),
        (
            "planets",
            &PLANETS,
...
fn write_file(file: &str, bodies: &[Box<dyn Body + Sync>], generators: &[Generator], data: &Data) {
...
fn generate_code(bodies: &[Box<dyn Body + Sync>], generators: &[Generator], data: &Data) -> String {
