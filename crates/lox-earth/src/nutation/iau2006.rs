...
pub fn nutation_iau2006a(centuries_since_j2000_tdb: JulianCenturies) -> Nutation {
    ...
    nutation.longitude.0 += nutation.longitude.0 * (0.4697e-6 + j2_correction);
    nutation.obliquity.0 += nutation.obliquity.0 * j2_correction;
...
mod tests {
    ...
    fn test_nutation_iau2006a_jd0() {
        ...
        assert_float_eq!(0.00000737285641780423, actual.longitude.0, rel <= TOLERANCE);
        assert_float_eq!(0.00004132905772755788, actual.obliquity.0, rel <= TOLERANCE);
    }
    ...
    fn test_nutation_iau2006a_j2100() {
        ...
        assert_float_eq!(0.00001585983730501046, actual.longitude.0, rel <= TOLERANCE);
        assert_float_eq!(0.00004162315218980551, actual.obliquity.0, rel <= TOLERANCE);
    }
}
