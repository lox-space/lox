...
def noise_temp_link_budget(t_sys_k, rx_gain_db=30.0):
    ...
    rx = lox.RxChain(
        ...
        lox.NoiseTempReceiver(noise_temperature=t_sys_k * lox.K),
        band=WIDE_BAND,
...
def test_tdrs_ka_band_return_link():
    ...
    tx = lox.TxChain(
        tx_antenna,
        lox.AmplifierTransmitter(power=tx_power_w * lox.W),
        band=WIDE_BAND,
    ...
    front_end = lox.CascadeReceiver.from_lna_and_noise_figure(
